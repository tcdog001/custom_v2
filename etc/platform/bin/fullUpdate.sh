...
CURRENT_MEDIA_VERSION=fullUpdateCurrentMediaVersion.json
CURRENT_ROUTE_VERSION=fullUpdateCurrentRouteVersion.json
COOKIE_FILE=fullUpdateCookie.txt
PAGE=lteFullUpdate.do
NEW_VERSION_INFO=fullUpdateNewVersionInfo.json
NEW_MEDIA_VERSION=fullUpdateNewMediaVersion.json
NEW_ROUTE_VERSION=fullUpdateNewRouteVersion.json
MEDIA_VERSION_PATH=/mnt/flash/rootfs_data/version/md/
ROUTE_VERSION_PATH=/mnt/flash/rootfs_data/version/ap/
versionName=upgrade.tar.gz
is_media_update=0
is_route_update=0
CURLE_OK=0
...
if [ -f $FILE_REGISTER ];then
	...
	type=`cat $NEW_VERSION_INFO |jq '.version[0].type'`
	echo type=$type
	if [ x$type == x"media" ];then
		cat $NEW_VERSION_INFO |jq '.version[0]' > $NEW_ROUTE_VERSION
		cat $NEW_VERSION_INFO |jq '.version[1]' > $NEW_MEDIA_VERSION
	else
		cat $NEW_VERSION_INFO |jq '.version[0]' > $NEW_MEDIA_VERSION
		cat $NEW_VERSION_INFO |jq '.version[1]' > $NEW_ROUTE_VERSION
	fi
	rm $NEW_VERSION_INFO
	...
	if [ $is_media_update -eq 1 ];then
		...
		echo "media_version_info:"$usr--$passwd--$url--$src--$name--$versionNo
	...
	if [ $is_route_update -eq 1 ];then
		...
		echo "route_version_info:"$usr--$passwd--$url--$src--$name--$versionNo
		for i in $(seq 1 $CONTINUE_TIMES); do
			curl -u $usr:$passwd -s -C - -o $versionName ftp://$url/$src/$name
			response=$?
			if [ $response -eq $CURLE_OK ];then
